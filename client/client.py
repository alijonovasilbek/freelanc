...
from .schemes import GigCategoryPost, GigSkillPost, GigFilePost,GigSkill,GigFile,GigCategoryfull,GigFilefull,GigSkillfull,Gigfull
...
async def get_gig_categories(gig_id: int, token: dict = Depends(verify_token), session: AsyncSession = Depends(get_async_session)):
...
async def delete_gig_category(
    ...
    result = await session.execute(select(gigs_category).where(gigs_category.c.id == category_id, gigs_category.c.gigs_id == gig_id))
...
async def get_gig_skill(gig_id: int, token: dict = Depends(verify_token), session: AsyncSession = Depends(get_async_session)):
...
async def create_gig_skill(
    ...
    gig_data = result.fetchone()
    if not gig_data:
        raise HTTPException(status_code=403, detail="Siz bu gigga skill qo'sha olmaysiz, chunki bu sizning gigingiz emas")
...
async def delete_gig_skill(
    ...
    result = await session.execute(select(gigs_skill).where(gigs_skill.c.id == skill_id, gigs_skill.c.gigs_id == gig_id))
...
@router_public.get('/gigss', response_model=List[Gig], summary="Get all Gigs aaa User")
